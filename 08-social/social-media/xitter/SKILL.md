...
| Tier | Price | Read Limit | Post Tweets |
|------|-------|------------|-------------|
| **Free** | $0 | 500/month | ❌ **NOT ALLOWED** |
| Basic | $100/month | 10K/month | ✅ Yes |
| Pro | $5000/month | 1M/month | ✅ Yes |
...
2. **Enable OAuth 1.0a** (NOT OAuth 2.0) in User authentication settings
3. Set permissions to `Read and write`
4. Callback URL: `http://localhost:8000/callback` (for CLI tools)
5. Website URL: `https://x.com/your_username`
6. Generate or regenerate the access token + access token secret after enabling write permissions
7. Save all five values carefully — missing any one of them will usually produce confusing auth or permission errors
...
```python
import urllib.parse
decoded = urllib.parse.unquote("YOUR_ENCODED_TOKEN")
```
...
- **HTTP 402 CreditsDepleted**: Your Free tier account cannot post tweets. Must upgrade to Basic ($100/mo) or use browser automation.
- **HTTP 401 Unauthorized (Bearer token)**: Check if token has URL-encoded characters that need decoding.
- **Paid API access**: many failures are plan/permission problems, not code problems.
- **OAuth 2.0 vs OAuth 1.0a**: `x-cli` requires OAuth 1.0a (Consumer Key + Access Token). OAuth 2.0 (Client ID only) won't work.
...
## Alternatives (Free Posting)

If you need to post without paying $100/month:

| Platform | API Cost | Notes |
|----------|----------|-------|
| **Bluesky** | Free | Open protocol, official API, growing fast |
| **Mastodon** | Free | Open source, self-hostable, many instances |
| **Nostr** | Free | Decentralized, no API limits |
| Browser Automation | Free | Use Playwright/Selenium to automate x.com (risky, can break) |
